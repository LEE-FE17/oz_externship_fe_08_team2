...
`useSuspenseQuery`를 사용하는 상세 페이지에서 삭제 직후 쿼리 캐시를 제거하면 리페치 → 404 → ErrorBoundary로 이동하는 문제가 발생했습니다.  
캐시를 유지한 채 성공 토스트를 표시하고, 토스트가 닫힐 때 `useRef`에 저장된 목적지로 이동하는 방식으로 해결했습니다.
